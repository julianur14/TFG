# -*- coding: utf-8 -*-
"""
Created on Mon Jan  2 12:46:05 2023
...
def inverse_transform_sampling(n_samples, lower_lim, upper_lim, min_value, max_value, poly_degree, p_val):
    
    # Extract values from the cdf function
    price_range = np.linspace(lower_lim, upper_lim, n_samples)
    cdf_values = np.array([cdf(x, min_value, max_value, poly_degree, p_val) for x in price_range])
...
def cdf(x, min_value, max_value, poly_degree, p_val):
    probs = np.array([unnorm_pdf(x, min_value, max_value, poly_degree) for x in p_val])
    norm_const = sampling_norm_constant_q2_p1(probs)
    avail_p = p_val[p_val < (x + 0.001)]
    cum_probs = np.sum( np.array([norm_const * unnorm_pdf(y, min_value, max_value, poly_degree) for y in avail_p]) )
    return cum_probs
